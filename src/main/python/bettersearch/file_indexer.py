...
from .database.util import create_init_config, is_sql_query, format_sqlrows_to_text, format_sqlrows_to_dict

from .database.parse import parse_file_contents
...
class WindowsFileIndexer:
    def __init__(self, vector_db_path: str = "better_search_content_db", check_interval: int = 30, device: str = "cpu", **kwargs):
        ...
        Args:
            ...
            **kwargs: Additional keyword arguments for the DBInterface initialization.
        """
        self.conn = OleDb.connect(constants.WIN_CONN_STRING)
        self.vector_db = DBInterface(vector_db_path=os.path.join(BASE_DIR, vector_db_path), device=device, **kwargs)
