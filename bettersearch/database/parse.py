...
from bettersearch.util import convert_gps_info_to_lat_lon_alt, get_all_exts
...
def parse_file(file_path):
    ...
    if pathlib.Path(file_path).suffix not in get_all_exts(parsable_exts):
        ...
        elif ext in parsable_exts.get('ffmpeg'):
...
def _parse_ffmpeg(file_path, ext):
    if ext in (parsable_exts.get('ffmpeg_audio')+parsable_exts.get('ffmpeg_video')):
        ...
        audio_metadata, video_metadata = None, None
    ...
    # if 
    return (image_metadata, video_metadata, audio_metadata)
